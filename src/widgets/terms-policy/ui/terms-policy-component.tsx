import cls from './terms-policy-component.module.scss'

import { ArrowBackIcon } from '@/shared/assets/icons/arrow-left'
import { getRegistrationRoute } from '@/shared/consts/route-paths'
import { useTranslation } from '@/shared/hooks/use-translation'
import { Button } from '@/shared/ui/button'
import { Card } from '@/shared/ui/card'
import { Typography } from '@/shared/ui/typography'

export interface TermsAndPolicyComponentProps {
  isPolicy?: boolean
}

export const TermsAndPolicyComponent: React.FC<TermsAndPolicyComponentProps> = props => {
  const { isPolicy } = props
  const { t } = useTranslation()

  return (
    <>
      <Button variant="link" href={getRegistrationRoute()} as="a" className={cls.backButton}>
        <ArrowBackIcon className={cls.buttonIcon} />
        <Typography color="secondary" variant="regular_14" clickable>
          {t.auth.backToRegistration}
        </Typography>
      </Button>
      <Card width="huge" variant="custom">
        <Typography variant="h1" align="center" className={cls.title}>
          {isPolicy ? t.auth.privacyPolicyTitle : t.auth.termsOfServiceTitle}
        </Typography>
